...
StageKit is a small standalone .NET infrastructure library with five projects in `StageKit.slnx`: `StageKit` (core application infrastructure), `StageKit.Primitives` (dependency-light reusable helpers), `StageKit.Runtime` (runtime and entry-application helpers), `StageKit.Demo` (console demo), and `StageKit.Tests`. Within `StageKit`, `Extensions/` and `Interfaces/` are namespace folders (not separate packages) holding small supporting APIs (`CollectionExtensions`, `DateTimeExtensions`, `ExceptionExtensions`, `ISavable`). The core pieces interlock through `ApplicationKit` static configuration:
...
- **`StageKit.Demo`** — runnable console demo (`dotnet run --project StageKit.Demo/StageKit.Demo.csproj`) covering startup configuration, Serilog integration, AutoSave settings, collection settings, panic-save registration, crash-report launch handling, and debounced save waiting. Not packed/published.
...
- **Storage utilities**: `ApplicationBackup` creates/restores profile zip backups. `SupportBundleExporter` writes manifest/config/log support bundles. `ApplicationRetention` applies log file and crash report retention. `OnboardingStateFile` persists first-run/onboarding completion state as a `RootSettingsFile<T>`. Atomic writes and low-level IO helpers live in `StageKit.Primitives/IO` as `SafeFile`, `SafeFileStream`, `PathUtilities`, `TemporaryDirectory`, and `TemporaryFile`. `StageKit.Primitives` also has a disposal-pattern family: `DisposableObject` (managed + unmanaged cleanup, no finalizer) → `UnmanagedDisposableObject` (adds finalizer fallback for types owning unmanaged resources directly) and `LeaveOpenDisposableObject`; `GCSafeHandle` and `UnmanagedMemoryManager` round out native-memory interop helpers.
